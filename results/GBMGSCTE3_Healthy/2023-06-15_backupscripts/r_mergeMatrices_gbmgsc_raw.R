...
ge_original <- readRDS("~/scratch/temp/analysis/2023-03-20/ge_gsc_raw.rds")
print("Imported wang_gsc unfiltered seurat object as ge_original")
gte_original <- readRDS("~/scratch/temp/analysis/2023-03-20/gte_gsc_raw.rds")
print("Imported wang_gsc unfiltered seurat object as gte_original")
...
gte <- CreateSeuratObject(counts = inter, 
                            ...
                            min.features = 100,
                            min.cells = 1)
gte_combined <- merge(gte, y=gte_original, 
...
ge <- CreateSeuratObject(counts = ge_mtx[,cellnames], 
                            ...
                            min.features = 100,
                            min.cells = 1)
ge_combined <- merge(ge, y=ge_original, 
                        add.cell.ids = c("GBM", "GSC"),
...
ge_meta <- ge_meta %>% add_column(sampleID = "")
gte_meta <- gte_meta %>% add_column(sampleID = "")
