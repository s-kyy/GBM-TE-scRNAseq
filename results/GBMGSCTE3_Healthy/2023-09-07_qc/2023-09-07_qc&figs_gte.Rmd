---
title: "Healthy ASD Control Samples pre-filtered figures (GTE)"
date: 2023-08-09
...
### Filter based on cells present in ge.

```{r}
# Load barcodes matching hg38-mapped object
barcodes <- read.csv("D:\\backup files\\getegbm\\gete-gbm\\results\\GBMGSCTE3_Healthy\\2023-09-07_qc\\healthy_ge_qc_barcodes.csv")
barcodes <- barcodes$x
subset_data <- subset(data, cells=barcodes)
...
write.csv(df, file="healthy_gte_samplecounts_qc.csv", row.names = F)
```

Save filtered dataset

```{r}
saveRDS(subset_data, file = "D:\\backup files\\getegbm\\gete-gbm\\results\\GBMGSCTE3_Healthy\\2023-09-07_qc\\gte_healthy_qc.rds")
```
...
p
ggsave("fig_gte_cellcount-bar_qc.tiff", plot = p, units="in", width=size*1, height=size*0.6, dpi=300, compression = 'lzw')
...
p
ggsave("fig_healthy_gte_umigenemitoratio_intercepts_qc.tiff", plot = p, units="in", width=size*1.5, height=size*1.5, dpi=300, compression = 'lzw')
...
ggsave("fig_healthy_gte_mitoratio_density_qc.tiff", plot = p, units="in", width=size*1.1, height=size*0.6, dpi=300, compression = 'lzw')
...
p
ggsave("fig_healthy_gte_log10GenesUMI_density_qc.tiff", plot = p, units="in", width=size*1.1, height=size*0.6, dpi=300, compression = 'lzw')
...
```{r Load-GTE}
data <- readRDS("D:\\backup files\\getegbm\\gete-gbm\\results\\GBMGSCTE3_Healthy\\2023-09-07_qc\\gte_healthy_qc.rds") 
print(head(DefaultAssay(data)))
DefaultAssay(data) <- "RNA"
...
for(sample in 1:length(data.list)) {
  ...
  filename <- paste(".\\variablefeatures_gte\\fig_gte_variablefeatures_",samplenames[sample],".tiff", sep="")
...
```{r}
saveRDS(data.integrated, file ="D:\\backup files\\getegbm\\gete-gbm\\results\\GBMGSCTE3_Healthy\\2023-09-07_qc\\gte_healthy_integrated.rds")
```
...
```{r}
saveRDS(data.dimred, file ="D:\\backup files\\getegbm\\gete-gbm\\results\\GBMGSCTE3_Healthy\\2023-09-07_qc\\gte_healthy_dimred.rds")
```
...
ggsave("fig_healthy_gte_elbowplot.tiff", plot = p, units="in", width=size*0.7, height=size*0.7, dpi=300, compression = 'lzw')
...
write.csv(varExplained, file="healthy_gte_pca_varExplained.csv", row.names = T)
...
Within all 25 PCs in summary they explain \~20% of the variation in our current dataset, which isn't a lot.. I expected at least 50%.
...
```{r Load-GTE}
data.dimred <- readRDS("D:\\backup files\\getegbm\\gete-gbm\\results\\GBMGSCTE3_Healthy\\2023-09-07_qc\\gte_healthy_dimred.rds") 
print(head(DefaultAssay(data.dimred)))

colnames(data.dimred@meta.data)

sampleIDs <- sort(unique(data.dimred$sampleID))
print(sampleIDs)

# Factor sample names
data.dimred$sampleID <- factor(data.dimred$sampleID, levels = sampleIDs)
```

```{r}
# Cell Count to csv
DefaultAssay(data.dimred) <- "RNA"
df <- data.dimred@meta.data %>% count(sampleID) 
df$genes.cell <- (dim(data.dimred)[1])/df$n
write.csv(df, file="healthy_gte_cellcounts_generatio.csv", row.names = F)

df <- data.dimred@meta.data %>% count(integrated_snn_res.0.3) 
write.csv(df, file="healthy_gte_cellcountscluster0.3.csv", row.names = F)

df <- data.dimred@meta.data %>% count(integrated_snn_res.0.4) 
