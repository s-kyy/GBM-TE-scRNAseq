...
```{r Load-GE}
data <- readRDS("D:\\backup files\\2021-11-11\\Cluster\\scratch\\gete-gbm\\results\\GBM-GSC neuroblastoma TE\\2021-09-02\\ge_gbmscIntUmap-subtypes.rds")
...
ggsave("fig_ge_cellcount-bar.tiff", plot = p, units="in", width=size*0.85, height=size*0.7, dpi=300, compression = 'lzw')

DefaultAssay(data) <- "RNA"
df <- data@meta.data %>% count(sampleCombined) 
write.csv(df, file="ge_cellcounts.csv", row.names = F)

df <- data@meta.data %>% count(integrated_snn_res.0.3) 
write.csv(df, file="ge_cellcountscluster0.3.csv", row.names = F)

df <- data@meta.data %>% count(integrated_snn_res.0.4) 
write.csv(df, file="ge_cellcountscluster0.4.csv", row.names = F)

df <- data@meta.data %>% select(integrated_snn_res.0.3, gbm_subtype) %>% count(integrated_snn_res.0.3, gbm_subtype) 
write.csv(df, file="ge_cellcountssubtypes.csv", row.names = F)

# PCA Elbow Plot
p <- ElbowPlot(data)
ggsave("fig_ge_elbowplot.tiff", plot = p, units="in", width=size*0.7, height=size*0.7, dpi=300, compression = 'lzw')

## Genes x UMI (Density plot)
p <- data@meta.data %>% 
	ggplot(aes(color=sampleCombined, x=log10GenesPerUMI, fill=sampleCombined)) + 
	geom_density(alpha = 0.2) + 
  scale_color_manual(values = samplePalette) +
  scale_fill_manual(values = samplePalette) +
  labs(fill="Samples", color="Samples", y = "Cell Density", x = "log10 Genes per UMI") +
	theme_classic() +
	geom_vline(xintercept = 0.8, linetype = "dashed", colour = "red") 
p
ggsave("fig_gbmgsc_ge_log10GenesUMI_density.tiff", plot = p, units="in", width=size*0.9, height=size*0.5, dpi=300, compression = 'lzw')
...
ggsave("fig_ge_cellsxClusterxSamples_PC20r03.tiff", plot = p, units="in", width=size*0.6, height=size*0.6, dpi=300, compression = 'lzw')
...
p <- df %>% ggplot(aes(x=integrated_snn_res.0.4, y=rounded, fill=sampleCombined)) + 
        ...
        labs(y = "Cells per Cluster per Sample (%)", x = "hg38 Mapped Cluster ID", fill= "Samples") +
        theme(axis.title = element_text(size=10), legend.position = "none")
ggsave("fig_ge_cellsxClusterxSamples_PC20r04.tiff", plot = p, units="in", width=size*0.6, height=size*0.6, dpi=300, compression = 'lzw')
...
ggsave("fig_ge_PC20r03_UMAP.tiff", plot = p, units="in", width=size*1.1, height=size, dpi=300, compression = 'lzw')
...
DimPlot(data, label=TRUE) 
p <- DimPlot(data, label=FALSE) 
ggsave("fig_ge_PC20r04_UMAP.tiff", plot = p, units="in", width=size*1.1, height=size*1, dpi=300, compression = 'lzw')

# Samples
Idents(data) <- "sampleCombined"
p <- DimPlot(data, label=FALSE, cols = samplePalette) 
ggsave("fig_ge_UMAP_samplesCombined.tiff", plot = p, units="in", width=size*1.2, height=size*1, dpi=300, compression = 'lzw')

# Cell Types
  # Move celltype labels to meta files
celltypeMeta <- read.csv("D:\\backup files\\getegbm\\gete-gbm\\results\\GBM-GSC neuroblastoma TE\\2022-11-09_celltypes_gbmsc\\gbmsc_meta_celltypes.csv", row.names = 1)
...
ggsave("fig_ge_UMAP_celltypes.tiff", plot = p, units="in", width=size*1.3, height=size*1, dpi=300, compression = 'lzw')

df <- data@meta.data %>% select(integrated_snn_res.0.3, int03_celltypes) %>% count(integrated_snn_res.0.3, int03_celltypes) 
write.csv(df, file="ge_celltypecounts.csv", row.names = F)

# GBM subtypes
data$gbm_subtype <- factor(data@meta.data$gbm_subtype, levels = gbmsubtypes, labels = gbmsubtypes_labels)
Idents(data) <- "gbm_subtype"
p <- DimPlot(data, label=FALSE, cols = gbmPalette) 
ggsave("fig_ge_UMAP_gbmsubtypes.tiff", plot = p, units="in", width=size*1.1, height=size*1, dpi=300, compression = 'lzw')
```
...
ggsave("fig_ge_dotplot_int03_celltypes_summary.tiff", plot = p, units="in", width=size*1.1, height=size*1.3, dpi=300, compression ='lzw')

# GBM subtypes (smaller size for the paper)
