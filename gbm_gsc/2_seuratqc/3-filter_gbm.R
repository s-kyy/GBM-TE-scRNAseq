...
for (i in 1:length(ge_list)) {
  ...
  temp_list[[i]] <- subset(
    x=ge_list[[i]], 
    subset= (nUMI >= 1000) & # remove cells with transient background level reads
      (nGene >= 300) & 
      (log10GenesPerUMI > 0.80) & 
      (mitoRatio < 0.20) # nuclei should have no mitochondrial genes (with some margin)
  ) 
...
for (i in 1:length(gte_list)) {
  ...
  temp_list[[i]] <- subset(
    x=gte_list[[i]], 
    subset= (nUMI >= 1000) & # remove cells with transient background level reads
      (nGene >= 300) & # remove cells with transient background level reads
      (log10GenesPerUMI > 0.80) & 
      (mitoRatio < 0.20) # nuclei should have no mitochondrial genes (with some margin)
  ) 
